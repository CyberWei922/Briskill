...
enum SkillWorkflowDescriber {
    static func actionLines(for definition: SkillWorkflowDefinitionV3) -> [String] {
        definition.steps.enumerated().map { index, step in
            let name = ToolDescriptorCatalog.byIdentifier[step.toolID]?.displayName ?? step.toolID
            return "\(index + 1). \(name)"
        }
    }
    ...
    static func bindingDescription(
        ...
        case .userInput:
            return "完整输入"
        case .parameter(let identifier):
            return parameters.first(where: { $0.id == identifier })?.name ?? "已删除的参数"
        case .stepOutput(let identifier):
            guard let index = steps.firstIndex(where: { $0.id == identifier }) else { return "未知步骤输出" }
            let name = ToolDescriptorCatalog.byIdentifier[steps[index].toolID]?.displayName ?? "步骤"
            return "第 \(index + 1) 步 · \(name)"
        case .literal(let value):
            let text = value.displayText
            return text.isEmpty ? "空内容" : String(text.prefix(48))
        case .template:
            return "文本模板"
        case .array(let values):
            return "\(values.count) 项内容"
        case .object(let values):
            return "\(values.count) 个字段"
        }
    }
}
