...
struct SkillEditorView: View {
    ...
    private func permissionDisplayName(_ permission: String) -> String {
        ...
        case "screen_recording": "屏幕录制"
        case "user_selected_file": "所选文件"
        case "user_selected_folder": "所选文件夹"
        case "downloads_write": "下载目录"
        case "process_information": "进程信息"
        case "apple_events": "自动化控制"
        case "cloud_api": "云端模型"
        case "confirmation_required": "执行前确认"
        case "risk_acknowledgement_required": "脚本风险确认"
        default: permission
        }
    }
...
private struct WorkflowStepCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(descriptor?.displayName ?? step.toolID)
                        .font(.system(size: 11.5, weight: .semibold))
                    Text(stepSummary)
                        .font(.system(size: 9.5))
                        .foregroundStyle(.secondary)
                                    ...
                                    if !priorSteps.isEmpty {
                                        Section("步骤输出") {
                                            ForEach(priorSteps) { priorStep in
                                                Button(ToolDescriptorCatalog.byIdentifier[priorStep.toolID]?.displayName ?? priorStep.toolID) {
    ...
    private var stepSummary: String {
        guard !step.arguments.isEmpty else { return descriptor?.summary ?? "没有输入参数" }
        let values = step.arguments.keys.sorted().compactMap { name -> String? in
            ...
            let label = descriptor?.arguments.first(where: { $0.name == name })?.summary ?? name
            return "\(label)：\(value)"
        }
                    ...
                    if !priorSteps.isEmpty {
                        Section("步骤输出") {
                            ForEach(priorSteps) { step in
                                Text(ToolDescriptorCatalog.byIdentifier[step.toolID]?.displayName ?? step.toolID)
...
private func toolSymbol(_ identifier: String) -> String {
    ...
    case "selection.readText", "selection.replaceText": "text.cursor"
