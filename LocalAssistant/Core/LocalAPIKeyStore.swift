...
enum APIKeyStorageMode: String, CaseIterable, Identifiable {
    ...
    var detail: String {
        switch self {
        case .keychain:
            "由 macOS 钥匙串保护，默认选项；应用可以使用，但界面不提供明文显示。"
        case .localFile:
            "以仅当前用户可读的本地 JSON 保存，可在应用中再次显示；安全性低于钥匙串。"
        }
    }
}
...
enum LocalAPIKeyStore {
    ...
    static func storageURL(createDirectory: Bool = false) throws -> URL {
        ...
        let directory = baseURL.appendingPathComponent("LocalAssistant/Secrets", isDirectory: true)
        if createDirectory {
