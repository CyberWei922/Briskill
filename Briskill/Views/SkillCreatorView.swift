...
private func localizedCreatorText(_ key: String) -> String {
...
struct SkillCreatorView: View {
    ...
    @State private var outputText = String(localized: "在助手面板显示，并复制到剪贴板")
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if mode == .guided {
                HStack {
                    ...
                    .disabled(guidedStep == 0 || isGenerating)

                    Spacer()

                    Button("下一步") {
                        withAnimation(.easeInOut(duration: 0.18)) {
                            guidedStep = min(3, guidedStep + 1)
                        }
                    }
                    .buttonStyle(.plain)
                    .font(.system(size: 11.5, weight: .medium))
                    ...
                    .contentShape(Rectangle())
                    .disabled(guidedStep == 3 || isGenerating)
                }
                .padding(.horizontal, 24)
                .padding(.bottom, 10)
            }
    ...
    private var guidedForm: some View {
        VStack(alignment: .leading, spacing: 18) {
            GuidedStepHeader(
                ...
                accentColor: AppAccent.resolve(appAccent).color
            ) { step in
                guidedStep = step
            }
    ...
    private var freeformEditor: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            TextEditor(text: $freeformText)
                ...
                .padding(12)
                .frame(maxWidth: .infinity, minHeight: 78, idealHeight: 92, maxHeight: 108)
                .background(Color.primary.opacity(0.045), in: RoundedRectangle(cornerRadius: 16, style: .continuous))
                .overlay(alignment: .topLeading) {
                    if freeformText.isEmpty {
                        Text("例如：以后我输入“会议收尾”并粘贴会议记录，就把文字总结成待办，按优先级排序，然后复制到剪贴板。")
                            .font(.system(size: 13))
    ...
    private var parameterValidationError: String? {
        for (index, parameter) in parameters.enumerated()
        where parameter.name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return String(localized: "第 \(index + 1) 个参数没有填写名称。")
        }
        ...
        for parameter in parameters {
            ...
            if !names.insert(name).inserted {
                return String(localized: "参数名称“\(parameter.name)”重复，请为每个参数使用不同名称。")
            }
        }
        return nil
    }

    private var guidedCompletedSteps: Set<Int> {
        var completed = Set<Int>()
        if !skillName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
           keywordValidationError == nil {
            completed.insert(0)
        }
        if !parameters.isEmpty, parameterValidationError == nil {
            completed.insert(1)
        }
        if !processText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            completed.insert(2)
        }
    ...
    private var creationValidationIssue: (message: String, step: Int?)? {
        if skillName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return (String(localized: "请填写技能名称。"), mode == .guided ? 0 : nil)
        }
        if let keywordValidationError {
            return (keywordValidationError, mode == .guided ? 0 : nil)
        }
        if let parameterValidationError {
            return (parameterValidationError, mode == .guided ? 1 : nil)
        }
        switch mode {
        case .guided:
            if processText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                return (String(localized: "请填写技能的处理过程。"), 2)
            }
            if outputText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                return (String(localized: "请填写技能的输出形式和内容。"), 3)
            }
        case .freeform:
            if freeformText.trimmingCharacters(in: .whitespacesAndNewlines).count < 8 {
                return (String(localized: "请完整描述技能需求（至少 8 个字符）。"), nil)
            }
        }
        return nil
    }
    ...
    private var primaryActionTitle: String {
        String(localized: "生成草稿")
    }

    private func performPrimaryAction() {
        if let issue = creationValidationIssue {
            message = CreatorMessage(text: issue.message, details: nil, isError: true)
            if let step = issue.step {
                withAnimation(.easeInOut(duration: 0.18)) {
                    guidedStep = step
                }
            }
    ...
    private func generate() {
        guard creationValidationIssue == nil else { return }
        ...
        generationTask = Task {
            do {
                if settings.isConfigured() {
                    ...
                    generationEndedAt = Date()
                    draft = localDraft
                    generatedBy = String(localized: "本地模板")
                    generatedInputFingerprint = inputFingerprint
                    phase = .preview
                ...
                generationEndedAt = Date()
                phase = .cancelled
                message = CreatorMessage(text: String(localized: "生成已由用户终止"), details: nil, isError: false)
    ...
    private var keywordValidationError: String? {
        ...
        guard !value.isEmpty else { return String(localized: "唯一索引不能为空") }
        guard !value.contains(where: \.isWhitespace), !value.hasPrefix("-") else {
            return String(localized: "唯一索引不能包含空格，也不能以“-”开头")
        }
        ...
        return isDuplicate ? String(localized: "唯一索引“\(value)”已被其他技能使用") : nil
    }
    ...
    private func startNewPage() {
        ...
        parameters = []
        processText = ""
        outputText = String(localized: "在助手面板显示，并复制到剪贴板")
        freeformText = ""
    ...
    private var formattedGenerationDuration: String {
        guard let generationStartedAt else { return String(localized: "0.0 秒") }
        let end = generationEndedAt ?? Date()
        return String(
            format: String(localized: "%.1f 秒"),
            end.timeIntervalSince(generationStartedAt)
        )
    }
    ...
    private func saveDraft() {
        ...
        do {
            ...
            message = CreatorMessage(
                text: String(localized: "“\(skill.name)”已保存，现在可以在快捷面板中搜索。"),
                details: nil,
                isError: false
            )
        } catch {
            message = CreatorMessage(
                text: String(localized: "保存失败"),
    ...
    private func diagnosticDetails(for error: Error) -> String {
        ...
        let providerLabel = String(localized: "服务商")
        let modelLabel = String(localized: "模型")
        let endpointLabel = String(localized: "API 地址")
        let timeLabel = String(localized: "时间")
        return """
        \(error.localizedDescription)

        \(providerLabel)：\(provider)
        \(modelLabel)：\(configuration.model)
        \(endpointLabel)：\(configuration.endpoint)
        \(timeLabel)：\(Date().formatted(date: .numeric, time: .standard))
        """
    }
}
...
private struct GenerationProgressView: View {
    ...
    private var reasoningDisplayText: String {
        ...
        if !thinkingEnabled {
            return switch phase {
            case .generating: String(localized: "Think 深度思考未开启，模型正在直接生成技能定义…")
            case .cancelled: String(localized: "生成已终止。")
            case .failed: String(localized: "生成失败，没有可显示的过程信息。")
            default: ""
            }
        }
        return switch phase {
        case .generating: String(localized: "正在等待 \(provider) 返回思考过程…")
        case .cancelled: String(localized: "生成在收到思考内容前被终止。")
        case .failed: String(localized: "模型没有返回可显示的思考过程。")
        default: ""
        }
    }

    private func elapsedText(at date: Date) -> String {
        guard let startedAt else { return String(localized: "0.0 秒") }
    ...
    private var statusTitle: String {
        switch phase {
        case .generating: String(localized: "正在生成技能")
        case .cancelled: String(localized: "生成已终止")
        case .failed: String(localized: "生成失败")
        default: String(localized: "生成技能")
        }
    }
...
private struct CreationModePicker: View {
    ...
    private func modeButton(
        ...
        return Button {
            ...
            Label(localizedCreatorText(title), systemImage: symbol)
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(localizedCreatorText(title))
        ...
        HStack(spacing: 7) {
            ForEach(titles.indices, id: \.self) { index in
                ...
                Button {
                    ...
                    .foregroundStyle(
                        isCompleted
                            ? (isCurrent ? Color.white : accentColor)
                            : (isCurrent ? Color.primary : Color.secondary)
                    )
                    .frame(maxWidth: .infinity, minHeight: 34)
                    .background(
                        isCompleted
                            ? (isCurrent ? accentColor : accentColor.opacity(0.09))
                            : Color.primary.opacity(isCurrent ? 0.075 : 0.035),
...
private struct CompactCreatorField: View {
    let title: String
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            TextField(localizedCreatorText(placeholder), text: $text)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            TextEditor(text: $text)
                ...
                .overlay(alignment: .topLeading) {
                    if text.isEmpty {
                        Text(localizedCreatorText(placeholder))
...
private struct PromptField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                Text(localizedCreatorText(keyword))
            ...
            TextField(localizedCreatorText(prompt), text: $text, axis: .vertical)
                .textFieldStyle(.plain)
                .lineLimit(1...3)
                .padding(.horizontal, 12)
                .padding(.vertical, 10)
