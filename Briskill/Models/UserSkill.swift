...
enum SkillExecutionMode: String, Codable, CaseIterable, Identifiable {
    ...
    var displayName: String {
        ...
        case .cloud: String(localized: "云端")
        }
    }

    var compactDescription: String {
        switch self {
        case .local: String(localized: "仅使用本机工具、AppleScript 或本地模型")
        case .hybrid: String(localized: "组合本机能力与用户选择的云端模型")
        case .cloud: String(localized: "只处理用户直接提交给云端模型的内容")
        }
    }
...
enum SkillParameterType: String, Codable, CaseIterable, Identifiable {
    ...
    var displayName: String {
        ...
        case .paragraph: String(localized: "段落文字")
        ...
        case .boolean: String(localized: "是 / 否")
        }
    }
...
extension String {
    func replacingSkillParameterPlaceholders(_ replacements: [String: String]) -> String {
        ...
        let staged = replacements.keys.sorted().enumerated().map { index, oldName in
            (oldName, "__BRISKILL_PARAMETER_\(index)__")
        }
