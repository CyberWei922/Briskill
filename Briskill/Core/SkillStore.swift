...
enum SkillStoreError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unsupportedFutureSchema(let version):
            "这个技能使用了较新的格式（v\(version)），请升级 Briskill 后再编辑"
        case .protectedDefinitionConflict:
            "磁盘上存在同一 ID 的未识别技能文件。为避免覆盖原始数据，当前版本拒绝保存"
        case .storageRequiresNewerVersion(let version):
            "技能索引使用了较新的格式（v\(version)）。当前版本已进入只读保护，请升级 Briskill"
        }
    }
}
...
struct BriskillSkillPackage: Codable {
    static let formatIdentifier = "com.briskill.skill"
...
final class SkillStore: ObservableObject {
    ...
    private let retiredBuiltInIdentifiers: Set<String> = [
        "builtin.file.move",
        "builtin.selection.summarize",
        "builtin.selection.translate",
        "builtin.selection.explain",
        "builtin.selection.rewrite"
    ]
    ...
    func exportPackageData(for skill: UserSkill) throws -> Data {
        ...
        let package = BriskillSkillPackage(
            format: BriskillSkillPackage.formatIdentifier,
            schemaVersion: BriskillSkillPackage.currentSchemaVersion,
            exportedAt: Date(),
        ...
        do {
            ...
            if fileManager.fileExists(atPath: v3Directory.path) {
                ...
                skills = result.skills
                    .map(migratingStoredSkill)
                    .map(localizingBuiltInSkill)
    ...
    private func migratingStoredSkill(_ storedSkill: UserSkill) -> UserSkill {
        ...
        synchronizeModelTask(in: &skill, definition: definition)
    ...
    /// Built-in definitions are stored in a stable, language-neutral form, then
    /// localized only after their ID-based workflow has been migrated. Unknown
    /// text is returned unchanged, so a user's own edits are never translated or
    /// replaced accidentally.
    private func localizingBuiltInSkill(_ storedSkill: UserSkill) -> UserSkill {
        guard storedSkill.isBuiltIn,
              let identifier = storedSkill.builtInIdentifier,
              let canonicalSkill = BuiltInSkillCatalog.skills.first(where: { $0.builtInIdentifier == identifier })
        else { return storedSkill }
        var skill = storedSkill
        let sourceSummary = skill.summary

        let canonicalStrings = [
            canonicalSkill.name,
            canonicalSkill.summary,
            canonicalSkill.originalRequest,
            canonicalSkill.trigger,
            canonicalSkill.condition,
            canonicalSkill.fallback,
            canonicalSkill.output,
            canonicalSkill.explanation
        ].compactMap { $0 }
            + canonicalSkill.aliases
            + canonicalSkill.actions
            + (canonicalSkill.dataDisclosure ?? [])
            + canonicalSkill.resolvedParameters.flatMap { [$0.name, $0.description] }
            + [canonicalSkill.modelTask?.promptTemplate].compactMap { $0 }
            + canonicalSkill.resolvedWorkflowV3.steps.compactMap(\.promptTemplate)

        let englishBundle = Bundle.main.path(forResource: "en", ofType: "lproj").flatMap(Bundle.init(path:))
        let englishToCanonical = Dictionary(
            canonicalStrings.map { source in
                let english = englishBundle?.localizedString(forKey: source, value: source, table: nil) ?? source
        ...
        func localized(_ value: String) -> String {
            let canonical = englishToCanonical[value] ?? value
            return Bundle.main.localizedString(forKey: canonical, value: canonical, table: nil)
        }

        skill.name = localized(skill.name)
        skill.aliases = skill.aliases.map(localized)
        skill.summary = localized(skill.summary)
        if skill.originalRequest == sourceSummary {
            skill.originalRequest = skill.summary
        } else {
            skill.originalRequest = localized(skill.originalRequest)
        }
        let canonicalTrigger = canonicalSkill.trigger
        let keyword = skill.registeredKeyword ?? skill.name
        let triggerFormatKey = skill.resolvedParameters.isEmpty ? "输入 %@" : "输入 %@，并提供所需参数"
        let englishTriggerFormat = englishBundle?.localizedString(
            forKey: triggerFormatKey,
            value: triggerFormatKey,
            table: nil
        ) ?? triggerFormatKey
        let englishTrigger = String(format: englishTriggerFormat, keyword)
        if skill.trigger == canonicalTrigger || skill.trigger == englishTrigger {
            let localizedTriggerFormat = Bundle.main.localizedString(
                forKey: triggerFormatKey,
                value: triggerFormatKey,
                table: nil
            )
            skill.trigger = String(format: localizedTriggerFormat, keyword)
        } else {
            skill.trigger = localized(skill.trigger)
        }
        skill.condition = skill.condition.map(localized)
        skill.actions = skill.actions.map(localized)
        skill.fallback = skill.fallback.map(localized)
        skill.output = localized(skill.output)
        let canonicalExplanation = canonicalSkill.explanation
        let explanationSuffixKey = "这是随软件提供的可编辑默认技能；它与用户技能使用相同格式，可以关闭、修改、导出或删除。"
        let englishExplanationSuffix = englishBundle?.localizedString(
            forKey: explanationSuffixKey,
            value: explanationSuffixKey,
            table: nil
        ) ?? explanationSuffixKey
        if skill.explanation == canonicalExplanation
            || skill.explanation.hasSuffix(explanationSuffixKey)
            || skill.explanation.hasSuffix(englishExplanationSuffix) {
            let localizedExplanationSuffix = Bundle.main.localizedString(
                forKey: explanationSuffixKey,
                value: explanationSuffixKey,
                table: nil
            )
            skill.explanation = skill.summary + " " + localizedExplanationSuffix
        } else {
            skill.explanation = localized(skill.explanation)
        }
        skill.dataDisclosure = skill.dataDisclosure?.map(localized)
        if var modelTask = skill.modelTask {
            modelTask.promptTemplate = localized(modelTask.promptTemplate)
            skill.modelTask = modelTask
        }
        if var workflowV3 = skill.workflowV3 {
            for index in workflowV3.steps.indices {
                workflowV3.steps[index].promptTemplate = workflowV3.steps[index].promptTemplate.map(localized)
            }
            skill.workflowV3 = workflowV3
        }
        skill.parameters = skill.parameters?.map { parameter in
            var parameter = parameter
            parameter.name = localized(parameter.name)
            parameter.description = localized(parameter.description)
            return parameter
        }
        return skill
    }
    ...
    private func synchronizeModelTask(
        in skill: inout UserSkill,
        definition: SkillWorkflowDefinitionV3
    ) {
        guard let firstModelStep = definition.steps.first(where: { $0.toolID == "model.generateText" }),
              let promptTemplate = firstModelStep.promptTemplate,
              !promptTemplate.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            if !definition.steps.contains(where: { $0.toolID == "model.generateText" }) {
                skill.modelTask = nil
            }
            return
        }
        skill.modelTask = SkillModelTask(
            ...
            inputVariables: skill.resolvedParameters.map(\.id) + ["userInput"],
            providerPolicy: skill.modelTask?.providerPolicy ?? "userDefault"
        )
    }
    ...
    private func removeRetiredBuiltInSkills() {
        ...
        do {
            ...
            AppConsole.shared.info(
                String(
                    format: String(localized: "已移除 %lld 个不再提供的内置技能"),
                    retiredSkills.count
                ),
                category: "SkillStore"
            )
    ...
    private func storageURL(createDirectory: Bool) throws -> URL {
        ...
        let directory = baseURL.appendingPathComponent("Briskill/Skills", isDirectory: true)
        if createDirectory {
            try fileManager.createDirectory(at: directory, withIntermediateDirectories: true)
        }
...
private enum BuiltInSkillCatalog {
    static let skills: [UserSkill] = [
        ...
        textModelSkill(
            number: 17,
            identifier: "builtin.text.translate",
            name: "翻译文字",
            keyword: "trans",
            aliases: ["translate", "文字翻译"],
            summary: "翻译用户直接输入的文字；中文翻译为自然英语，其他语言翻译为中文。",
            instruction: "翻译下面的文字。如果原文主要是中文，翻译成自然英语；否则翻译成自然中文。保留段落和语气，只输出译文。",
            output: "在面板中显示译文"
        ),
        textModelSkill(
            number: 18,
            identifier: "builtin.text.explain",
            name: "解释文字",
            keyword: "explain",
            aliases: ["文字解释", "ex"],
            summary: "解释用户直接输入的文字，并用容易理解的方式说明。",
            instruction: "解释下面的文字。先给出一句话结论，再说明关键概念；根据原文难度控制篇幅。",
            output: "在面板中显示通俗解释"
        ),
        textModelSkill(
            number: 19,
            identifier: "builtin.text.rewrite",
            name: "改写文字",
            keyword: "rewrite",
            aliases: ["润色", "文字改写"],
            summary: "改写用户直接输入的文字，使表达更清晰、自然、专业。",
            instruction: "改写下面的文字，使表达更清晰、自然、专业。保持原意、语言和必要格式，只输出改写后的正文。",
            output: "在面板中显示改写结果"
        ),
        make(
            number: 7,
    ...
    private static func textModelSkill(
        number: Int,
        identifier: String,
        name: String,
        keyword: String,
        ...
        make(
            ...
            aliases: aliases,
            summary: summary,
            actions: ["接收用户输入的文字", "调用用户选择的云端模型处理", "显示结果"],
            output: output,
            requiredTools: ["model.generateText"],
            permissions: ["cloud_api"],
            executionMode: .cloud,
            parameters: [parameter("text", "文字内容", .paragraph, "要交给模型处理的完整文字")],
            workflow: [
                step("generate", "model.generateText", ["input": .string("{{文字内容}}")], saveAs: "result")
            ],
            modelTask: modelTask(instruction + "\n\n{{text}}", variables: ["text"]),
            dataDisclosure: ["用户输入的文字会发送给用户选择的云端模型用于处理"]
        )
    }
