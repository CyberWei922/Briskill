...
enum ToolExecutionError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .emptyClipboard:
            String(localized: "剪贴板中没有可处理的文字。请先复制一段文本后再运行技能。")
        case .clipboardWriteFailed:
            String(localized: "翻译已经生成，但写入剪贴板失败。请重试或手动复制结果。")
        case .missingArgument(let tool, let argument):
            String(localized: "工具 \(tool) 缺少参数 \(argument)。")
        case .unresolvedWorkflowReference(let reference):
            String(localized: "工作流引用 \(reference) 没有可用的运行时值。请检查技能参数和步骤顺序。")
        case .unsupportedWorkflowVersion(let version):
            String(localized: "这项技能使用了较新的工作流格式（V\(version)）。请升级 Briskill 后再运行。")
        case .missingWorkflow:
            String(localized: "这个技能还没有可执行的工作流。")
        case .unavailableTools(let tools):
            String(localized: "尚未接入这些本地工具：\(tools.joined(separator: "、"))")
        case .localSkillRequestedCloudModel:
            String(localized: "本地执行技能不能调用云端模型。请在技能设置中将执行模式改为“云端 AI”。")
        case .unsupportedFileInput:
            String(localized: "当前文本模型工具不会发送文件路径或文件内容；请先使用文件读取或 OCR 工具转换为文本。")
        case .emptyToolResult(let tool):
            String(localized: "工具 \(tool) 没有返回可用结果。")
        case .screenRecordingPermissionRequired:
            String(localized: "需要“屏幕录制”权限才能进行区域截图。请在设置 → 隐私与权限中授权 Briskill；系统可能要求重新启动应用。")
        case .invalidPath(let path):
            String(localized: "无法访问路径：\(path)")
        case .unsupportedFileType(let type):
            String(localized: "暂不支持读取这种文件：\(type)")
        case .fileTooLarge(let limitMB):
            String(localized: "文件过大。当前单个文件读取上限为 \(limitMB) MB。")
        case .fileOperationFailed(let reason):
            String(localized: "文件操作失败：\(reason)")
        case .operationCancelled:
            String(localized: "用户取消了操作。")
        case .screenCaptureCancelled:
            String(localized: "没有完成区域截图。")
        case .ocrFailed:
            String(localized: "没有从图片中识别到文字。")
        }
    }
}
...
private final class ScreenCaptureRegionTool: AssistantTool {
    ...
    func execute(
        ...
        let capturesDirectory = FileManager.default.urls(
            ...
            .appendingPathComponent("Briskill/Captures", isDirectory: true)
...
private final class ModelGenerateTextTool: AssistantTool {
    ...
    func execute(
        ...
        return ToolExecutionOutput(
            value: .string(readableResult),
            displayText: readableResult,
    ...
    func execute(
        ...
        for (index, step) in workflow.steps.enumerated() {
            ...
            do {
                ...
                if let usage = output.tokenUsage {
                    if let existing = tokenUsage {
                        tokenUsage = GenerationTokenUsage(
                            ...
                            reasoningTokens: existing.reasoningTokens + usage.reasoningTokens,
